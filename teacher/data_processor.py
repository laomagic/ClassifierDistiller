...
class DataPrecessForSentence(Dataset):
    ...
    def __init__(self, bert_tokenizer, file, max_char_len=100):
        """
        bert_tokenizer :分词器
        LCQMC_file     :语料文件
        """
    ...
    def get_input(self, file):
        ...
        # labels = [int(data[4]) for i in data]
        # 切词
        tokens_seq_1 = list(
...
if __name__ == '__main__':
    bert_tokenizer = BertTokenizer.from_pretrained('../lib/bert/')
    train_dataset = DataPrecessForSentence(bert_tokenizer, '../data/ranking/train.tsv')
    train_dataloader = DataLoader(train_dataset, batch_size=64, shuffle=True)
    pbar = tqdm(train_dataloader)
    for index, train in enumerate(pbar):
        if index ==1:
            break
        seq, seq_mask, seq_segment, label = train
        print(seq)
        print(seq_mask)
        print(seq_segment)
        print(label)
